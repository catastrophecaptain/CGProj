...
void TextureMapping::initBlendShader() {
    ...
    // -----------------------------------------------------------------
    const char* fsCode =R"(#version 330 core   
         in vec3 fPosition;   
         in vec3 fNormal;   
         in vec2 fTexCoord;   
         out vec4 color;   
         ...
         void main() {
             ...
             vec3 color_light_1 = light.color * light.intensity * lambertian * material.kds[0];
             vec3 color_light_2 = light.color * light.intensity * lambertian * material.kds[1];
             vec4 color_tex_1 = texture(mapKds[0], fTexCoord);
             vec4 color_tex_2 = texture(mapKds[1], fTexCoord);
             vec3 color_1 = color_light_1 * color_tex_1.rgb;
             vec3 color_2 = color_light_2 * color_tex_2.rgb;
             color = vec4(((1-material.blend)*color_1+color_2*material.blend), 1.0);
         })";
    //----------------------------------------------------------------
...
void TextureMapping::initCheckerShader() {
    ...
    const char* fsCode =
        ...
        void main() {
            if(int(material.repeat*fTexCoord.x)%2 == int(material.repeat*fTexCoord.y)%2) {
                color = vec4(material.colors[0], 1.0);
            } else {
                color = vec4(material.colors[1], 1.0);
            }
        })";
    //----------------------------------------------------------------
